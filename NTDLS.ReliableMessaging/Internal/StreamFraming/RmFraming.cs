...
namespace NTDLS.ReliableMessaging.Internal.StreamFraming
{
    ...
    internal static class RmFraming
    {
        ...
        public static bool ReadAndProcessFrames(this Stream stream, RmContext context, RmEvents.ExceptionEvent? onException, RmFrameBuffer frameBuffer,
        ...
        /// <summary>
        /// Writes a one-time fire-and-forget notification to the stream.
        /// </summary>
        /// <param name="stream">The open stream that will be written to.</param>
        /// <param name="context">Contains information about the endpoint and the connection.</param>
        ...
        private static byte[] AssembleFrame(RmContext context, RmFrameBody frameBody)
        {
            var frameBodyBytes = RmSerialization.SerializeToByteArray(frameBody);
            ...
            var grossFrameSize = frameBodyBytes.Length + RmConstants.GrossFrameHeaderSize;
            var grossFrameBytes = new byte[grossFrameSize];
            var frameCrc = RmCRC16.ComputeChecksum(frameBodyBytes);

            Buffer.BlockCopy(BitConverter.GetBytes(RmConstants.FrameDelimiter), 0, grossFrameBytes, 0, 4);
            ...
            Buffer.BlockCopy(frameBodyBytes, 0, grossFrameBytes, RmConstants.GrossFrameHeaderSize, frameBodyBytes.Length);
